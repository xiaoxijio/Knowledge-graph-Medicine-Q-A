...
class QuestionClassifier:
    def __init__(self):
        ...
        self.department_wds = [i.strip() for i in open(self.department_path, encoding="gbk") if i.strip()]
        self.check_wds = [i.strip() for i in open(self.check_path, encoding="gbk") if i.strip()]
        ...
        self.producer_wds = [i.strip() for i in open(self.producer_path, encoding="gbk") if i.strip()]
    ...
    def check_medical(self, question):
        ...
        stop_wds = []
        for wd1 in region_wds:  # 过滤重复词
            for wd2 in region_wds:
                if wd1 in wd2 and wd1 != wd2:
                    stop_wds.append(wd1)
        final_wds = [i for i in region_wds if i not in stop_wds]
        final_dict = {i: self.wdtype_dict.get(i) for i in final_wds}  # 获得当前词的类型
